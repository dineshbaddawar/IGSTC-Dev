    angular.module('cp_app').controller('ConsortialCtrl', function ($scope, $rootScope) {
        ...
        $scope.saveCVDetails = function () {
            ...
            for (let i = 0; i < $scope.publicationList.length; i++) {
                ...
                if($scope.contactDetails.Employment_Details__r != undefined){
                    delete $scope.contactDetails.Employment_Details__r;
                }
                if($scope.contactDetails.Education_Details__r != undefined){
                    delete $scope.contactDetails.Education_Details__r;
                }
                if($scope.contactDetails.Publications_Patents__r != undefined){
                    delete $scope.contactDetails.Publications_Patents__r;
                }                

            ApplicantPortal_Contoller.insertContactDetails($scope.contactDetails, $scope.educationList, $scope.employmentList, $scope.publicationList, function (result, event) {
                if (event.status) {
                    debugger;
                    Swal.fire(
                        'Success',
